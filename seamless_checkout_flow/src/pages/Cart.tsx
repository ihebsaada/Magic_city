  ...
  const handleCheckout = async () => {
    if (cart.length === 0) {
      toast({
        ...
        description:
          "Aggiungi degli articoli al carrello prima di procedere al pagamento.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (data.url) {
        ...
        throw new Error(
          "Nessun URL di pagamento o ID ordine ricevuto dal backend"
        );
      }
    } catch (error: any) {
      console.error("Checkout error:", error);
      toast({
        ...
        description:
          error.message ||
          "Impossibile elaborare il pagamento. Riprova per favore.",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 py-8">
        <div className="container mx-auto px-4 max-w-3xl">
          <Card className="animate-fade-in">
            <CardHeader>
              <CardTitle className="text-2xl">Il tuo carrello</CardTitle>
            </CardHeader>
